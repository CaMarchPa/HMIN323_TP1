...
def kdtree_representation(kdtree, bbox=[[-1., 1.],[-1., 1.],[-1., 1.]], radius=0.05):
    import numpy as np
...
def view_kdtree(kdtree, bbox=[[-1., 1.],[-1., 1.],[-1., 1.]], radius=0.05):
   import openalea.plantgl.all as pgl
   sc =kdtree_representation(kdtree, bbox, radius)
...
def kdtree_closest(point, kdtree, depth=0):
    ...
    if isinstance(kdtree, KDNode):
        ...
        if best_distance > abs(pcoord - kdtree.pivot[axis]):
