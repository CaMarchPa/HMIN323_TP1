...
def createkdtree(point_list, minbucketsize = 3, depth = 0):
	if len(point_list) > 2*minbucketsize :
		...
		node.pivot = p_med
		node.axis = axis
		node.left_child = createkdtree(point_list[:len(point_list)/2], minbucketsize, depth+1)
		node.right_child = createkdtree(point_list[len(point_list)/2 + 1:], minbucketsize, depth+1)
		return node
	else:
		return [list(pt) for pt in point_list]
